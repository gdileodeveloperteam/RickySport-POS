...
router.post('/',
  requirePermission({ modulo: 'DEVOLUCIONES', nivel: 2 }),
  async (req, res, next) => {
    try {
...
router.post('/cambio',
  requirePermission({ modulo: 'CAMBIOS', nivel: 2 }),
  async (req, res, next) => {
    try {
...
router.get('/',
  requirePermission({ modulo: 'DEVOLUCIONES', nivel: 1 }),
  async (req, res, next) => {
    try {
...
router.get('/cambios/:guid',
  requirePermission({ modulo: 'CAMBIOS', nivel: 1 }),
  async (req, res, next) => {
    try {
...
router.get('/creditos/:guidCliente',
  requirePermission({ modulo: 'DEVOLUCIONES', nivel: 1 }),
  async (req, res, next) => {
    try {
...
router.get('/credito-by-devolucion/:guidDevolucion',
  requirePermission({ modulo: 'DEVOLUCIONES', nivel: 1 }),
  async (req, res, next) => {
    try {
